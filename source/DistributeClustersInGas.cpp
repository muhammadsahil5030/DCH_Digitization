      			...
      			}) 
{
	...
	// Get UniqueIDGenSvc for event-reproducible random seeding
...
StatusCode DistributeClustersInGas::initialize() {
	...
	// Read the configured detector name
	...
	// Retrieve the DCH-specific data extension from the detector element
	dch_data = detectorElement.extension<dd4hep::rec::DCH_info>();
	if (!dch_data || !dch_data->IsValid()) {
		error() << "No valid data extension was found for detector <<" 
			<< detectorName << ">>." << endmsg;
		return StatusCode::FAILURE;
	}
...
double DistributeClustersInGas::get_dNcldx_per_cm(double betagamma,
	const edm4hep::MCParticle& mc) const 
		{
			...
			// Reconstruct particle momentum from beta*gamma and mass
			...
			// Energy loss in MeV*cm^2/g
			const double dEdx_MeVcm2_per_g = BB::bethe_bloch(p_GeV, m_MeV, I_MeV);
			
			// Convert to MeV/cm using gas density
			const double dEdx_MeV_per_cm = dEdx_MeVcm2_per_g * m_GasDensity_g_cm3.value();

			// Convert deposited energy into mean number of primary ionization clusters per cm
                                    ...
                                    const edm4hep::EventHeaderCollection& headers) const
{
    ...
    struct StepSegment {
	    ...
	    TVector3 stepMid_cm;               // midpoint of the Geant4 step [cm]
	    TVector3 trackDir;                 // unit direction of the Geant4 step
	...
	//===============================================================================//
	//                     Accumulate all ionized path per cell                      //
	//===============================================================================//
    struct CellAccum {
    ...
    // Accumulate accepted Geant4 steps per cell
    std::unordered_map<uint64_t, CellAccum> cellMap;

    // Select Geant4 hits and accumulate all step segments per cell
    int loop_index = 0;
	...
	for (const auto& kv : cellMap) 
	{
	    ...
	    for (const auto& seg : cell.segments) {
			...
			const double lambda_electrons_per_cm = get_dNcldx_per_cm(bg, seg.mcParticle);
	    ...
	    // Build cumulative path-length table for this cell
	    std::vector<double> cumulativeLengths_mm;
	    cumulativeLengths_mm.reserve(cell.segments.size());

	    double running_mm = 0.0;
	    for (const auto& seg : cell.segments) {
		  	running_mm += seg.stepLength_mm;
		  	cumulativeLengths_mm.push_back(running_mm);
	    }
		//-----------------------------------------------------------------------//
		...
		for (size_t icl = 0; icl < clusterPositions_mm.size(); ++icl) {
			...
			// Find which Geant4 step contains this generated cluster
		  	auto it = std::lower_bound(cumulativeLengths_mm.begin(),
			 		cumulativeLengths_mm.end(), sGlobal_mm);
		  	if (it == cumulativeLengths_mm.end()) continue;

		  	const size_t iseg = std::distance(cumulativeLengths_mm.begin(), it);
		  	const auto& seg = cell.segments[iseg];

			// Convert merged-path coordinate into local coordinate inside the chosen step
		  	const double prevCum_mm = (iseg == 0) ? 
				0.0 : cumulativeLengths_mm[iseg - 1];
		  	const double sLocal_mm  = sGlobal_mm - prevCum_mm;
		  	const double sLocal_cm  = sLocal_mm * MM_TO_CM;

		  	// Reconstruct start point of this Geant4 step from midpoint and direction
		  	const TVector3 stepStart_cm =
		      		seg.stepMid_cm - 0.5 * (seg.stepLength_mm * MM_TO_CM) * seg.trackDir;

		  	// Place the cluster along the selected Geant4 step
		  	const TVector3 clusterPos_cm = stepStart_cm + sLocal_cm * seg.trackDir;
